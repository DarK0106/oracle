...
select name || ',' || capital || ',' || population as "[국가정보]"
    from tblCountry;
...
select first_name || last_name as "풀네임", phone_number
    from employees
        where job_id = 'IT_PROG';

-- 9.employees
-- last_name이 'Grant'인 직원의 이름, 연락처, 고용날짜를 가져오시오.
select first_name || last_name as "풀네임", phone_number, hire_date
    from employees
        where last_name = 'Grant';

-- 10.employees
-- 특정 매니저(manager_id: 120)이 관리하는 직원의 이름, 급여, 연락처를 가져오시오.
select first_name || last_name as "풀네임", salary, phone_number
    from employees
        where manager_id = 120;

-- 11.employees
-- 특정 부서(60, 80, 100)에 속한 직원들의 이름, 연락처, 이메일, 부서ID 가져오시오.
select first_name || last_name as "풀네임", phone_number, email, job_id
    from employees
